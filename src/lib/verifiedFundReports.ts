...
export function verifiedFundReports(state: StoreState, projects: Project[], asOf: string) {
  ...
  const pending: Bill[] = state.bills.filter(b => projects.some(p => p.id === b.projectId) && b.status !== 'REJECTED').flatMap(b => {
    const remaining = b.netPayable - tx.filter(r => r.kind === 'PAYMENT' && r.fields.billId === b.id).reduce((n, r) => n + Number(r.fields.amount), 0);
  ...
  return { government: buildFundReport(projects, installments, asOf), contractor };
}
