...
import type { PdfReportOptions, DocumentRecordOptions } from './pdfTypes';
...
export function buildPdfReport(opts: PdfReportOptions) {
...
export function buildDocumentRecord(opts: DocumentRecordOptions) {
  const doc = new jsPDF({ unit: 'pt', format: 'a4', compress: true });
  const pageWidth = doc.internal.pageSize.getWidth();
  const marginX = 48;
  doc.setFillColor(15, 42, 82);
