    ...
    <div className={cn('relative overflow-hidden rounded-md', className)}>
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/55 to-transparent px-2.5 pb-2 pt-6 text-white">
        <div className="flex items-center gap-1 text-[11px] font-semibold leading-tight">
          <MapPin size={11} className="shrink-0 text-emerald-400" /> {location}
        </div>
        <p className="mt-0.5 truncate text-[9.5px] leading-tight text-white/80">Lat {lat.toFixed(6)}, Long {lng.toFixed(6)}</p>
        <p className="text-[9.5px] leading-tight text-white/80">{dateLabel}, {timeLabel}</p>
      </div>
    </div>
  );
}
