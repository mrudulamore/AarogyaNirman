...
export function PendingWorkPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-4">
    ...
    <div className="flex flex-wrap gap-2" aria-label={uiText('Filter pending work')}>
      {[['ALL', 'All'], ['OVERDUE', 'Overdue'], ['approvals', 'Approvals'], ['milestones', 'Milestones'], ['inspections', 'Inspections'], ['defects', 'Defects'], ['controls', 'Documents & renewals']].map(([value, label]) => <button key={value} aria-pressed={filter === value} onClick={() => { setFilter(value); setLimit(20); }} className={`min-h-11 rounded-full border px-4 text-xs font-medium transition-colors ${filter === value ? 'border-blue-200 bg-blue-50 text-blue-800' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}>{uiText(label)}</button>)}
    </div>
...
export function PendingWork() {
  ...
  return <Link to="/dashboard/pending-work" className="pending-work-summary group mb-4 flex min-h-16 items-center gap-3 rounded-2xl border border-blue-100 bg-white px-4 py-3 transition-colors hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300">
