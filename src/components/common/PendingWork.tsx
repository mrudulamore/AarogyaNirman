...
export function PendingWork() {
  ...
  </Link><div className="grid gap-2 sm:grid-cols-3">{tasks.slice(0, 3).map(task => <Link key={task.id} to={`/projects/${task.projectId}?tab=${task.tab}${task.billId ? `&bill=${encodeURIComponent(task.billId)}` : ''}`} className="min-w-0 rounded-xl border border-blue-100 bg-white p-3 text-sm hover:bg-blue-50"><p className="break-words font-semibold text-slate-800">{task.title}</p><p className="mt-1 break-words text-xs text-slate-500">{s.projects.find(p => p.id === task.projectId)?.name}</p><p className="mt-2 text-xs text-blue-700">{task.due} · {uiText('Open')} →</p></Link>)}</div></div>;
}
