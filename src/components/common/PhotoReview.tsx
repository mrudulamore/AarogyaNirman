...
export function PhotoReview({ photo }: { photo: SitePhoto }) {
  ...
  const canReview = user && ['SUPERADMIN','PROJECT_MANAGER','EXECUTIVE_ENGINEER'].includes(user.role) && photo.uploadedById !== user.id && (photo.uploadedById || photo.uploadedBy !== user.name);
  ...
  return <section className="mt-4 space-y-3 rounded-2xl border border-blue-100 bg-blue-50/50 p-4">
    ...
    <p className="text-xs text-slate-600">{uiText('GPS location and photo approval are separate checks. Project managers, executive engineers and superadmins review site evidence.')}</p>
