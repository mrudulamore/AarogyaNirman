...
export function ConfirmDialog({ open, onOpenChange, title, description, confirmLabel = 'Confirm', destructive, onConfirm }: {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent title={uiText(title)} description={uiText(description)} size="sm">
        <DialogFooter>
...
function ButtonGhostClose({ onOpenChange }: { onOpenChange: (v: boolean) => void }) {
  useUiLanguage();
  return <button onClick={() => onOpenChange(false)} className="inline-flex h-9 items-center rounded-md border border-slate-300 bg-white px-4 text-sm font-medium text-slate-700 hover:bg-slate-50">{uiText("Cancel")}</button>;
}
