...
export function FundDisbursalReports({ projects, scopeLabel }: { projects: Project[]; scopeLabel: string }) {
  ...
  return <Card className="my-4">
    <CardHeader><CardTitle>Fund Disbursal Reports</CardTitle></CardHeader>
    <CardContent className="space-y-4">
      <p className="text-xs text-slate-500">Track government receipts and contractor payments separately. Government installment entries and release conditions are illustrative demo data reconciled to the prototype totals.</p>
      <div className="flex flex-wrap items-end gap-3">
        <label className="flex min-w-48 flex-1 flex-col gap-1 text-xs font-medium text-slate-600">Project
          <select className="h-9 rounded-md border border-slate-300 bg-white px-2" value={projectId} onChange={(e) => setProjectId(e.target.value)}>
            <option value="ALL">All projects in scope</option>{projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
        </label>
        <label className="flex flex-col gap-1 text-xs font-medium text-slate-600">As of date
          <input type="date" max={todayDate()} value={asOf} onChange={(e) => setAsOf(e.target.value)} className="h-9 rounded-md border border-slate-300 px-2" />
        </label>
        <Button variant="outline" onClick={exportPdf} disabled={!valid || !selected.length}><Download size={14} /> Export {kind === 'roadmap' ? 'Roadmap' : 'Report'} PDF</Button>
      </div>
      {!valid ? <p role="alert" className="text-sm text-red-600">Choose a valid date up to today.</p> : <>
        <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
          <KpiCard label="Government funds received" value={formatCurrency(government.total)} sub={`${government.received.length} installments received by ${asOf}`} />
          <KpiCard label="Contractors paid" value={formatCurrency(contractor.total)} sub={`${contractor.paid.length} paid bills with payment dates`} />
          <KpiCard label="Government releases planned" value={formatCurrency(government.planned)} sub={`${government.pending.length} installments outstanding`} />
          <KpiCard label="Contractor bills pending" value={formatCurrency(contractor.pending.reduce((sum, b) => sum + b.netPayable, 0))} sub="Current workflow; payment dates not scheduled" />
        </div>
        <div className="flex flex-wrap gap-2" aria-label="Fund report type">
          {(Object.keys(titles) as ReportKind[]).map((key) => <Button key={key} variant={kind === key ? 'primary' : 'outline'} aria-pressed={kind === key} onClick={() => setKind(key)}>{titles[key]}</Button>)}
        </div>
        {kind === 'contractor' && <p className="text-xs text-slate-500">Each paid bill is one recorded payment. Bank transfer installments and transaction references are not recorded in the current billing data. Undated payments are excluded from date-based totals.</p>}
        {kind === 'roadmap' && <p className="text-xs text-slate-500">Government targets are indicative and subject to approval. Cumulative funds include receipts through the selected date plus scheduled releases in date order. Contractor workflow statuses are current, not a historical snapshot; future payment dates are not yet recorded.</p>}
...
function ReportTable({ section }: { section: PdfSection }) {
  return <section>
    ...
    {section.rows.length ? <div className="max-h-96 overflow-auto rounded-md border border-slate-200">
      <Table><THead><Tr>{section.columns.map((c) => <Th key={c}>{c}</Th>)}</Tr></THead>
        <TBody>{section.rows.map((row, i) => <Tr key={i}>{row.map((cell, j) => <Td key={j}>{cell}</Td>)}</Tr>)}</TBody>
      </Table>
    </div> : <p className="rounded-md bg-slate-50 p-4 text-xs text-slate-500">No records for the selected project and date.</p>}
  </section>;
}
