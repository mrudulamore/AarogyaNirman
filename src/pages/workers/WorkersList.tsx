...
export function WorkersList() {
  ...
  return (
    <div>
      <PageHeader title={t('pages.workers.title')} description={t('pages.workers.desc', { count: workers.length })} />

      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <MiniStat label="On Site" value={present} icon={Users} />
        <MiniStat label="Skilled" value={skilled} />
        <MiniStat label="Unskilled" value={unskilled} />
        <MiniStat label="Safety Training Pending" value={trainingPending} tone="amber" />
      </div>

      <Card className="mb-4">
        <CardHeader><CardTitle>Daily Workforce Trend</CardTitle></CardHeader>
        <CardContent>
      ...
      <Card>
        <Table>
          <THead><Tr><Th>Worker</Th><Th>Role</Th><Th>Project</Th><Th>Skill</Th><Th>Shift</Th><Th>Attendance</Th><Th>Safety Training</Th><Th /></Tr></THead>
          <TBody>
            {filtered.map((w) => (
              <Tr key={w.id}>
                ...
                <Td>{w.attendanceStatus !== 'PRESENT' && <Button size="sm" variant="outline" onClick={() => { markAttendance(w.id, w.projectId, 'QR'); toast.success(`${w.name} checked in.`); }}><QrCode size={12} /> Check-in</Button>}</Td>
              </Tr>
            ))}
          </TBody>
        </Table>
...
function MiniStat({ label, value, icon: Icon, tone = 'default' }: { label: string; value: number; icon?: any; tone?: 'default' | 'amber' }) {
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-3.5">
      <p className="flex items-center gap-1 text-[10.5px] font-medium uppercase text-slate-400">{Icon && <Icon size={11} />} {label}</p>
