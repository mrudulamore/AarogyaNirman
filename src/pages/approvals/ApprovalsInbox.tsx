...
export function ApprovalsInbox() {
  ...
  return (
    <div>
      <PageHeader title={t('pages.approvals.title')} description={t('pages.approvals.desc')} />

      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <KpiCard label="Awaiting My Action" value={myPending.length} icon={ClipboardCheck} tone="amber" onClick={() => setStatusFilter('PENDING')} />
        <KpiCard label="Total Pending" value={approvals.filter((a) => a.status === 'PENDING').length} icon={Clock} />
        <KpiCard label="Approved" value={approvals.filter((a) => a.status === 'APPROVED').length} icon={CheckCircle2} tone="emerald" />
        <KpiCard label="Rejected" value={approvals.filter((a) => a.status === 'REJECTED').length} icon={XCircle} tone="red" />
      </div>
      ...
      <div className="mb-3">
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          ...
          <SelectContent>
            ...
            {['PENDING', 'APPROVED', 'REJECTED', 'SENT_BACK', 'CLARIFICATION_REQUESTED'].map((s) => <SelectItem key={s} value={s}>{s.replace('_', ' ')}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      ...
      <Card>
        <Table>
          <THead><Tr><Th>Type</Th><Th>Project</Th><Th>Amount</Th><Th>Submitted By</Th><Th>Date</Th><Th>Current Step</Th><Th>Status</Th></Tr></THead>
          <TBody>
            {filtered.map((a) => (
              <Tr key={a.id} onClick={() => setDetailId(a.id)}>
                <Td className="font-medium text-slate-800">{a.type.replace(/_/g, ' ')}</Td>
                ...
                <Td>{a.status === 'PENDING' ? a.chain[a.currentStepIndex]?.replace(/_/g, ' ') : '—'}</Td>
      ...
      <Dialog open={!!detailId} onOpenChange={(v) => !v && setDetailId(null)}>
        {active && (
          <DialogContent title={active.type.replace(/_/g, ' ')} description={projects.find((p) => p.id === active.projectId)?.name} size="lg">
            <p className="text-xs text-slate-600">{active.comments}</p>
            ...
            <div className="space-y-2">
              ...
              {active.history.map((h, i) => (
                <div key={i} className="rounded-md border border-slate-100 bg-slate-50 p-2.5 text-xs">
                  <div className="flex justify-between"><span className="font-medium text-slate-700">{h.approver} · {h.designation}</span><StatusBadge status={h.decision} /></div>
                  <p className="mt-1 text-slate-500">{h.comment}</p>
                  <p className="mt-1 text-[10.5px] text-slate-400">{formatDateTime(h.timestamp)}</p>
                </div>
              ))}
            </div>
            {active.status === 'PENDING' && canDecide && (
              <div className="mt-4">
                <Textarea rows={2} placeholder="Add a comment (optional)" value={comment} onChange={(e) => setComment(e.target.value)} />
                <DialogFooter>
                  <Button variant="outline" onClick={() => act('CLARIFICATION_REQUESTED')}><MessageCircleQuestion size={13} /> Request Clarification</Button>
                  <Button variant="outline" onClick={() => act('SENT_BACK')}><RotateCcw size={13} /> Send Back</Button>
                  <Button variant="destructive" onClick={() => act('REJECTED')}><X size={13} /> Reject</Button>
                  <Button variant="success" onClick={() => act('APPROVED')}><Check size={13} /> Approve</Button>
                </DialogFooter>
              </div>
            )}
            {active.status === 'PENDING' && !canDecide && (
              <div className="mt-4 flex items-center gap-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs text-slate-500">
                <Lock size={13} className="shrink-0" /> Awaiting action from {ROLE_LABELS[active.chain[active.currentStepIndex]]}. This step isn't assigned to your role.
              </div>
            )}
          </DialogContent>
        )}
