  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatBox label={uiText("Pending Bills")} value={pending} />
        <StatBox label={uiText("Approved / Paid")} value={approved} />
        <StatBox label={uiText("Rejected")} value={rejected} />
        <StatBox label={uiText("Budget Utilization")} value={`${Math.round((project.amountSpent / project.sanctionedBudget) * 100)}%`} />
      </div>
      ...
      {project.amountSpent > project.sanctionedBudget * 0.9 && (
        <div className="rounded-md bg-amber-50 px-4 py-2.5 text-xs text-amber-700">{uiText("Budget utilization has crossed 90% — monitor remaining scope closely.")}</div>
      )}
    </div>
  );
}
...
function StatBox({ label, value }: { label: string; value: string | number }) {
  useUiLanguage();
  return <div className="rounded-lg border border-slate-200 bg-white p-3.5"><p className="text-[10.5px] font-medium uppercase text-slate-400">{uiText(label)}</p><p className="mt-1 text-2xl font-bold text-slate-800">{uiText(value)}</p></div>;
}
