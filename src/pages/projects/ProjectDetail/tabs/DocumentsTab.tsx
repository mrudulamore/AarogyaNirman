  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <div className="flex h-9 items-center gap-2 rounded-md border border-slate-300 bg-white px-2.5">
            <Search size={13} className="text-slate-400" /><input value={q} onChange={(e) => setQ(e.target.value)} placeholder={uiText("Search documents…")} className="w-40 text-xs outline-none" />
          </div>
          <Select value={typeFilter} onValueChange={setTypeFilter}>
            <SelectTrigger className="w-48"><SelectValue /></SelectTrigger>
            <SelectContent><SelectItem value="ALL">{uiText("All Types")}</SelectItem>{DOC_TYPES.map((t) => <SelectItem key={t} value={t}>{uiText(t)}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        {!readOnly && <Button onClick={() => setUploadOpen(true)}><Upload size={15} />{uiText(" Upload Document")}</Button>}
      </div>

      <Card>
        {filtered.length === 0 ? (
          <EmptyState icon={<FileText size={32} />} title={uiText("No documents found")} />
        ) : (
          <Table>
            <THead><Tr><Th>{uiText("Document")}</Th><Th>{uiText("Type")}</Th><Th>{uiText("Version")}</Th><Th>{uiText("Uploaded By")}</Th><Th>{uiText("Date")}</Th><Th>{uiText("Size")}</Th><Th>{uiText("Status")}</Th><Th /></Tr></THead>
            <TBody>
              {filtered.map((d) => (
                <Tr key={d.id}>
      ...
      <Dialog open={uploadOpen} onOpenChange={setUploadOpen}>
        <DialogContent title={uiText("Upload Document")} description={uiText(project.name)}>
          <div className="space-y-3">
            <div><p className="mb-1 text-xs font-medium text-slate-600">{uiText("Document Name")}</p><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
            <div>
              <p className="mb-1 text-xs font-medium text-slate-600">{uiText("Document Type")}</p>
