...
export function ApprovalsTab({ project }: { project: Project }) {
  ...
  return (
    <div className="space-y-4">
      <Card>
        {approvals.length === 0 ? <EmptyState icon={<ClipboardCheck size={32} />} title="No approval requests for this project" /> : (
          <Table>
            <THead><Tr><Th>Type</Th><Th>Amount</Th><Th>Submitted By</Th><Th>Date</Th><Th>Current Step</Th><Th>Status</Th></Tr></THead>
            <TBody>
              {approvals.map((a) => (
      ...
      <Dialog open={!!detailId} onOpenChange={(v) => !v && setDetailId(null)}>
        {active && (
          <DialogContent title={active.type.replace(/_/g, ' ')} description={project.name} size="lg">
            <div className="space-y-2 text-xs">
              <p className="text-slate-600">{active.comments}</p>
