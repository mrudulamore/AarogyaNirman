...
import { assessProjectGeoFence, GEOFENCE_RADIUS_M } from '../../../../lib/geo';
...
export function FieldEvidenceTab({ project }: { project: Project }) {
  ...
  const rows = photos.map((p) => {
    ...
    const status = p.isReference ? assessment.status : p.geoFenceStatus ?? assessment.status;
    return { photo: p, distanceM: p.isReference ? assessment.distanceM : p.distanceFromSiteM ?? assessment.distanceM, status, within: status === 'INSIDE' };
  ...
  const viewerRow = rows.find(row => row.photo.id === viewerId);
  const viewerIndex = filtered.findIndex(r => r.photo.id === viewerId);
  const flaggedCount = rows.filter((r) => !r.photo.isReference && (!r.within || r.photo.locationSource === 'MANUAL')).length;
...
{viewerPhoto.isReference ? <DemoPhotoDetails photo={viewerPhoto} project={project} /> : <>
            <div className="mt-3 grid grid-cols-2 gap-2 rounded-md bg-slate-50 p-3 text-xs sm:grid-cols-4">
              ...
              <div>
                ...
                {viewerPhoto.locationSource === 'MANUAL' ? <StatusBadge status="PENDING" label={uiText('Location unverified')} /> : viewerRow?.status === 'UNCERTAIN' ? (
...
function EvidenceRadar({ rows, radiusM, polygon }: { rows: { photo: SitePhoto; distanceM: number; within: boolean; status: string }[]; radiusM: number; polygon: boolean }) {
  useUiLanguage();
  ...
  return <div className="space-y-4">
    ...
    <div className="max-h-72 space-y-4 overflow-y-auto pr-2">{rows.map(({photo,distanceM,within,status}) => <div key={photo.id}>
      ...
      <div className="relative h-2 rounded-full bg-slate-100"><div className={`h-2 rounded-full ${photo.isReference ? 'bg-slate-400' : photo.locationSource === 'MANUAL' || status === 'UNCERTAIN' ? 'bg-amber-400' : within ? 'bg-blue-500' : 'bg-red-400'}`} style={{width: `${Math.max(1,distanceM / maximum * 100)}%`}}/>{!polygon && <span className="absolute -top-1 h-4 w-0.5 bg-slate-500" style={{left:`${radiusM / maximum * 100}%`}}/>}</div>
    </div>)}</div>
